...
\begin{algorithm}[H]
    ...
    \Blankline
    Normalize samples to have a maximum amplitude of $1$\;
    Apply Butterworth LPF to samples\;
    Find where each sample first crosses a threshold value\;
    Zero-pad the appropriate sample to align the threshold indices\;
    Truncate the longer sample to match the length of the shortest sample\;
    Run \emph{immse}
\end{algorithm}
