...
for i = 1:2:length(locations(:,1))
    for j = 1:2:length(input_samples_files(:,1))
        ...
        audiowrite(strcat(output_path_real, locations(i,:), input_samples_files(j+1,:)), real_output, fs_real, 'BitsPerSample', 32);
        ...
        audiowrite(strcat(output_path_sim, locations(i,:), input_samples_files(j+1,:)), sim_output, fs_sim, 'BitsPerSample', 32);
    end
end
more on;
